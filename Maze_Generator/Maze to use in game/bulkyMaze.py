...
class Player:
    ...
    def collidelist(self, list_):
        for wall in list_:
            temp = pygame.Rect(
                (int(self.x)+self.velX, int(self.y)+self.velY), self.size)
...
# Image
print(os.path.join(os.getcwd(), 'Maze to use in game', "screenshot1.png"))
image = pygame.image.load(os.path.join(os.getcwd(), "screenshot1.png"))
    ...
    while j-10 < len(mapy[0])-1:
        try:
            if list(mapy[i][j]) == [0, 0, 0] and list(mapy[i][j+9]) == [0, 0, 0]:
                lineList.append(pygame.draw.line(
                    win, (0, 0, 0), (j, i), (j+9, i), 2))
            if list(mapy[i][j]) == [0, 0, 0] and list(mapy[i+9][j]) == [0, 0, 0]:
                lineList.append(pygame.draw.line(
                    win, (0, 0, 0), (j, i), (j, i+9), 2))
